...
# FlareSolverr ensure (will restart if unhealthy — safe no-op if already good)
ENSURE_FS="${ENSURE_FS:-/opt/hs-data-api/scripts/ensure-flaresolverr.sh}"
if [[ -x "$ENSURE_FS" ]]; then
  "$ENSURE_FS" 2>/dev/null || echo "(ensure-flaresolverr returned non-zero)"
else
  echo "(ensure-flaresolverr script not present)"
fi
