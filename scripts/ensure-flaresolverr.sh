    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if body.get("status") != "ok":
                ...
                return
            # functional probe (no proxy) inside the same client
            try:
...
# Restart decision is conservative: only if the solver does not even answer sessions.list.
# Functional probe failure is logged as detail but does not trigger container restart (avoids flapping).
ok = bool(d.get("ok"))
